...
export default function App() {
    ...
    const onSubmit = async (data: TLoginSchema) => {
        ...
        if (response.success) {
            toast.success(response.message || "Login successful", { id: tId });
            route.refresh();
            redirect("/");

        } else {
            toast.error(response.message || "Login failed", { id: tId });
        }
    }
            ...
            <div className="w-full max-w-lg p-8 bg-white rounded-2xl shadow-xl border border-gray-100 transition-all hover:shadow-2xl">
                <div className="space-y-6">
                    <div className="space-y-2">
                        <h1 className="text-3xl font-extrabold text-center tracking-tight text-gray-900">Welcome Back</h1>
                        <p className="text-center text-gray-500 text-sm">Enter your credentials to access your account</p>
                    </div>
                    <Form {...form} >
                        <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-5">
                            <div className="space-y-4">
                                <InputComponent form={form} name="email" placeholder="Enter your email" label="Email" icon={User} />
                                <PasswordComponent form={form} name="password" placeholder="Enter your password" label="Password" />
                            </div>
                            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-2">
                                ...
                                <div className="text-sm text-gray-600 text-center sm:text-right">
                                    ...
                                    <Link href="/register" className="font-semibold text-blue-600 hover:text-blue-700 hover:underline underline-offset-4 transition-all whitespace-nowrap">
                                        Register
                                    </Link>
                                </div>
                            </div>
